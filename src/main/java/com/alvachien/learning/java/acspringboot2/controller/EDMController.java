package com.alvachien.learning.java.acspringboot2.controller;
...
public class EDMController {
	...
	public ResponseEntity<String> process(HttpServletRequest req) {
		...
		try {
			...
			ODataHandler handler = new ODataHandler(odata, edm, null);
			...
			for (String key : response.getAllHeaders().keySet()) {
				headers.add(key, response.getHeaders(key).toString());
			}
